...
const projectsDir = path.join(process.env.USERPROFILE, '.aura_os_data', 'projects');
const projectsMetaPath = path.join(process.env.USERPROFILE, '.aura_os_data', 'projects_meta.json');

const projectsMeta = JSON.parse(fs.readFileSync(projectsMetaPath, 'utf8'));

fs.readdirSync(projectsDir).forEach(projId => {
  ...
  // If it has index.html but NO package.json, it's a 1995 dummy project!
  if (fs.existsSync(indexPath) && !fs.existsSync(packagePath)) {
    const htmlContent = fs.readFileSync(indexPath, 'utf8');
    if (htmlContent.includes('George has fully initialized')) {
      console.log('Upgrading project:', projId);
      
      const meta = projectsMeta.find(p => p.id === projId);
      const name = meta ? meta.name : 'Aura Project';
